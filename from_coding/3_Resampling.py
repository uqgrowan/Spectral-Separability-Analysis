...
#satellites = ["landsat_9"]

#for loop to resample for each satellite
for satellite in satellites:

	#calculate the total sensor response per band to be the denominator, filter to bands with non-negligible response
    resampling_SRF = choose_SRF(satellite, min_wavelength, max_wavelength)
    response_denom = resampling_SRF.sum(axis=0)
    resampling_SRF = resampling_SRF.loc[:, response_denom > 0.01]
    response_denom= response_denom[response_denom > 0.01]
