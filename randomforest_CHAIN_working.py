...
class Unmix_Pixels:
    ...
    def classify_and_append(self, classifier_params, plot):
        ...
        water_rfr = RandomForestRegressor().fit(self.x_train, self.y_train.iloc[:, -1])
    ...
    def filter_and_regress(self, plot, regressor_params):
        ...
        for col in range(len(self.y_train.columns[:-1])):
            ...
            test_predictions = reg.predict(self.x_test.loc[:, :100])
            train_predictions = reg.predict(self.x_train.loc[:, :100])
            
            class_predictions["train"][self.y_train.columns[col]] = pd.DataFrame(train_predictions, index = self.x_train.index)
            class_predictions["test"][self.y_train.columns[col]] = pd.DataFrame(test_predictions, index = self.x_test.index)
            
            regressors[self.y_train.columns[col]] = reg
            
            #Predict on only present pixels
            present_predictions_test = reg.predict(self.x_test.loc[:, :100])
