...
data_file_path = r"C:\Users\s4770224\Documents\coding\Spectral_analysis\Combined_analysis\MEL_NZ_TAS_spectra.csv"
labels_file_path = r"C:\Users\s4770224\Documents\coding\Spectral_analysis\Combined_analysis\All_reflectance_labels.csv"

labelled_data = ingest_and_clean(data_file_path, labels_file_path)

#Filter for just the classes, setup and sites of interest
keep_classes = assign_targets_list("farm") #could be all, kelp, browns, or farm    
keep_setup = ["handheld"]
keep_site = ["melbourne", "new_zealand", "tasmania"] 
subset = labelled_data[labelled_data["Class"].isin(keep_classes)]
subset = subset[subset["setup"].isin(keep_setup)]
subset = subset[subset["site"].isin(keep_site)]

# Make sort_order of classes
sort_order = ['ecklonia', 'macrocystis','petalonia','undaria','phyllospora', 'durvillaea', 'cystophora', 'acrocarpia', 'hormosira', 'carpophyllum',  'scytosiphon', 'sargassum','filamentous_rhodophyte', 'frondose_rhodophyte', 'ulva', 'grass','mussels','worm_castings', 'barnacle_shells', 'shell_litter','sand', 'gravel', 'rock',]

# Make a sort column with categorical type, sort to match the sort_order
subset["sort_column"] = subset["Class"]
subset['sort_column'] = pd.Categorical(subset['sort_column'], categories=sort_order, ordered=True)
subset = subset.sort_values('sort_column')

# Make data and labels dataframes
data = subset.select_dtypes(include = "number")
labels_new = subset.select_dtypes(include = "object")
labels_new.shape
resampling = "none_farm"
export_results = True
for c in range(0,1):
    ...
    plot_cm(num_labels, minimum_distance_class, fig_save_name)
