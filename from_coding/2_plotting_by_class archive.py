...
def assign_targets_list(group = "all"):
    if group == 'kelp':
        ...
        target_mask = all_labels["Class"].isin(target_classes)
    elif group == "all":
...
def import_data():  #choose paths here
    all_data = pd.read_csv(r"C:\Users\s4770224\Documents\coding\Spectral_analysis\Combined_analysis\Resampled\landsat_9.csv")
    all_labels = pd.read_csv(r"C:\Users\s4770224\Documents\coding\Spectral_analysis\Combined_analysis\New_reflectance_labels.csv")
...
target_mask = assign_targets_list("all")
targets_spectra = all_data[target_mask]
target_labels = all_labels[target_mask]
target_labels["colour"].value_counts()

#Normalize spectra to a range of 0 to 1
#std_df = standardize_data(targets_spectra, "handheld", 0, 15) #standardize to values between 125th to 300th bands
#std_df = std_df.copy() #copy to get rid of fragmentation?
std = targets_spectra
...
basic_group = spectra_to_use["colour"].unique()

stats_dict={}
#for each group, setup, site: make a dictionary entry with all of the statistics from calc_stats
for species in basic_group:
    spec_targets = spectra_to_use[spectra_to_use["colour"] == species]
    spec_targets = spec_targets.select_dtypes(include = "number")
    stats_dict[f"{species}"]= calc_stats(spec_targets)
...
#plotting function
def plot_search_terms(search_words, bad_bands_list, ymax = 5, ymin = 0.01, mode = "any", lowest_band = 325, highest_band = 1075, tick_interval = 100, options = options, stats_dict = stats_dict):
    ...
    if mode == "any":
        filtered_list = [item for item in options if any(word in item for word in search_words)]
    else:
        filtered_list = [item for item in options if all(word in item for word in search_words)]
    #else:
    ...
    for w in filtered_list:
        ...
        plotting_data.index = plotting_data.index+lowest_band
    ...
    plt.ylim(ymin, ymax)
    plt.yscale("log")
    plt.xticks([num for num in range(lowest_band, highest_band, tick_interval)])
...
search_words = ['brown', 'mineral', 'green', 'red']


#Call the plotting function according to the desired search words, mode, bad bands
masked = plot_search_terms(search_words, bad_bands_list, mode = "any", ymax = 30, ymin = 0.001, lowest_band= 0, highest_band= 6, tick_interval= 1)
