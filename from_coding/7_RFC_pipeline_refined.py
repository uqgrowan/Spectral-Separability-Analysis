...
def many_runs(runs, n_comps, filename, column, sensor_keys, indep, dep):
	...
	plt.savefig(rf"C:\Users\s4770224\Documents\coding\Spectral_analysis\Plots\For_publication\RFC_cms\RFC_weightedClasses_{sensor_keys[filename]}_{column}.svg")
	plt.show()
	return accuracies
...
column = 5
dep = sort_samples(labels).loc[class_mask].iloc[:,column]
...
n_comp = 6
indep = pd.read_csv(r'C:\Users\s4770224\Documents\coding\Spectral_analysis\Combined_analysis\Resampled\asd_MEL_NZ_TAS_spectra.csv')
indep = indep.loc[class_mask]
indep = sort_samples(indep)
indep = indep.drop(["Class", 'setup', 'site'], axis = 1) #.loc[class_mask]
many_run_results[f"{column}_{n_comp}"] = many_runs(100, n_comp, filename, column, sensor_keys, indep= indep, dep = dep)      
...
for column in range(5, 6): # labels.shape[1]):
